...
if __name__ == '__main__':
    ...
    db = Database(f"postgresql://{config.postgres_user}:{config.postgres_password}@postgres:{config.postgres_port}/{config.postgres_db}")
    
    shop_repository = ShopRepository(db)
    user_repository = UserRepository(db)

    authorization_repository = AuthorizationRepository(shop_repository, user_repository)
    authorization_use_case = AuthorizationUseCase(authorization_repository, config.jwt_secret_key)
